...
*/
var isValidBiosRegistry = function isValidBiosRegistry(target, registry) {
...
*/
var normalizeRBSUdata = function normalizeRBSUdata(data, isGen9, oem) {
    if (isGen9) {
        var unified_struct = {};
...
*/
var getSettingsURI = function getSettingsURI(rbsuData) {
    if (rbsuData['@Redfish.Settings'] && rbsuData['@Redfish.Settings'].SettingsObject) {
...
*/
var _create_conditional_statement = function _create_conditional_statement(obj) {
    var evalStr = '';
    ...
    evalStr = (obj.MapTerms ? obj.MapTerms : '') + ' (tempRBSU.Attributes["' + obj.MapFromAttribute + '"].' + obj.MapFromProperty + ' ' + obj.MapFromCondition + ' "' + obj.MapFromValue + '")';
    return evalStr;
};
...
*/
var _patchData = function _patchData(rbsuData, isGen9) {
    var returnData = null;
    if (isGen9) {
...
*/
var isViolating = function isViolating(data) {
    ...
    if (violate.length > 0) {
        var explanation = '[Invalid RBSU configuration]\n' + 'Please correct RBSU setting based on following suggestions:\n';
        for (var key in violate) {
            explanation += '[' + violate[key].name + '] should be [' + violate[key].value + '] because: \n';
            for (var index in violate[key].why) {
                var why = violate[key].why[index];
                explanation += '- ' + why.MapFromAttribute + '[' + why.MapFromProperty + '] ' + why.MapFromCondition + ' ' + why.MapFromValue + '\n';
            }
        }
        return explanation;
    } else {
...
var rbsu = (function () {
    ...
    function rbsu(client, password) {
        _classCallCheck(this, rbsu);
    ...
    _createClass(rbsu, [{
        ...
        value: function getRegistryContent(schemaData) {
            var registryUri = null;
            if (rbsu.gen9) {
            ...
            if (registryUri) {
                return rbsu.client.get(registryUri, null, null, rbsu.headers).then(function (res) {
                    return res.body;
                });
            } else {
                return null;
        ...
        */
        ...
        key: 'getRegistryOption',
        value: function getRegistryOption(rbsuData) {
            if (rbsuData.hasOwnProperty('AttributeRegistry')) {
                var AttributeRegistry = rbsuData.AttributeRegistry;
            }
            return rbsu.client.getRootObject(rbsu.headers).then(function (res) {
                ...
                for (var key in registryItems) {
                    if (isValidBiosRegistry(AttributeRegistry, registryItems[key]['@odata.id'])) {
        ...
        */
        ...
        key: 'getNextSetting',
        value: function getNextSetting(rbsuData) {
            var settingsURI = getSettingsURI(rbsuData);
            if (settingsURI) {
                return rbsu.client.get(settingsURI, null, null, rbsu.headers).then(function (res) {
        ...
        value: function getRBSUdata() {
            ...
            return rbsu.client.getRootObject(rbsu.headers).then(function (res) {
                ...
                if (res.body.Systems && res.body.Systems['@odata.id']) {
                    if (res.body.hasOwnProperty('Oem')) {
                        ...
                        if (Object.keys(res.body.Oem).length === 1) {
                            ...
                            if (res.body.Oem[oem]) {
                                if (res.body.Oem[oem].Manager && res.body.Oem[oem].Manager.length === 1) {
                                    if (res.body.Oem[oem].Manager[0].ManagerType === 'iLO 4') {
                ...
                res.body.Members.forEach(function (node) {
                    rbsuDataCollection.push(node);
                });
                var systemArray = rbsuDataCollection.map(function (obj) {
                    return rbsu.client.get(obj['@odata.id'], null, null, rbsu.headers);
                });
                return systemArray;
            }) //Resolve promise array of system node
            .then(function (systemArray) {
                return _bluebird2['default'].all(systemArray);
            }) //Get BIOS data from each system node
            .then(function (result) {
                var biosLinkArray = result.map(function (obj) {
                    try {
                        if (rbsu.gen9) {
                            return rbsu.client.get(obj.body.Oem[oem].Links.BIOS['@odata.id']);
                        } else {
                            if (obj.body.Bios && obj.body.Bios['@odata.id']) {
                                return rbsu.client.get(obj.body.Bios['@odata.id']);
                            }
                        }
                    } catch (error) {
                        throw error;
                ...
                return _bluebird2['default'].all(biosLinkArray);
            }).then(function (result) {
                rbsu.biosSetting = result.map(function (obj) {
                    if (obj && obj.body) {
        ...
        */
        ...
        key: 'validateRBSU',
        value: function validateRBSU(registry, currentRBSU, nextRBSU) {
            var returnData = {
                registry: {},
                RBSU: {},
                modified: []
            ...
            for (var key in attribute) {
                ...
                if (nextRBSU.Attributes[attribute[key][attributeName]] !== currentRBSU.Attributes[attribute[key][attributeName]]) {
                    tempRBSU.Attributes[attribute[key][attributeName]].CurrentValue = nextRBSU.Attributes[attribute[key][attributeName]];
                } else {
                    tempRBSU.Attributes[attribute[key][attributeName]].CurrentValue = currentRBSU.Attributes[attribute[key][attributeName]];
                }
            }
            ...
            for (key in dependency) {
                ...
                if (check) {
                    ...
                    if (tempRBSU.Attributes[mapTo.MapToAttribute]) {
                        checkStr = 'tempRBSU.Attributes["' + mapTo.MapToAttribute + '"]["' + mapTo.MapToProperty + '"] === ' + tmpValue;
                    } else {
                    ...
                    try {
                        var actionStr = checkStr.replace('===', '='),
                            force = false;
                        if (tempRBSU.Attributes[mapTo.MapToAttribute] && mapTo.MapToProperty === 'CurrentValue') {
                            if (tempRBSU.Attributes[mapTo.MapToAttribute].CurrentValue) {
                                force = true;
                            }
                        }
                        if (!eval(checkStr)) {
                            returnData.modified.push({
                                type: tempRBSU.Attributes[mapTo.MapToAttribute] ? 'Attributes' : 'Menus',
        ...
        key: 'updateRBSU',
        value: function updateRBSU(registry, currentRBSU, nextRBSU) {
            ...
            if (result) {
                ...
                return rbsu.client.patch(getSettingsURI(currentRBSU), _patchData(nextRBSU, rbsu.gen9));
            }
        }
    }, {
        key: 'addPassword',
        value: function addPassword(password) {
            return rbsu.client.setBiosPassword(password);
        }
    }, {
        key: 'changePassword',
        value: function changePassword(rbsuData, oldPassword, newPassword) {
            ...
            return passwordURL ? rbsu.client.patch(passwordURL, {
        ...
        value: function reset(rbsuData) {
            ...
            return resetURI ? rbsu.client.post(resetURI, {
                ResetType: 'default'
            }).then(function (res) {
                return res.body;
            }) : null;
